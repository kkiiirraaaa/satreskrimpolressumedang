...
export default function KringReserse() {
  const [kringReserse, setKringReserse] = useState<KringReserseType[]>([]);
  const [judul, setJudul] = useState("");
  const [deskripsi, setDeskripsi] = useState("");
  const [created_at, setCreated_At] = useState("");
  const [foto, setFoto] = useState<File | null>(null);
  const [showModal, setShowModal] = useState(false);
  const [selected, setSelected] = useState<KringReserseType | null>(null);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Upload ke bucket "kring-reserse"
      ...
      if (insertError) {
        ...
        alert("Kring Reserse Berhasil Ditambahkan!");
        setShowModal(false);
        setJudul("");
        setDeskripsi("");
        setFoto(null);
  ...
  const handleDelete = async (id: number) => {
    ...
    if (error) {
      ...
      setKringReserse((prev) => prev.filter((item) => item.id !== id));
    }
    if (!error) {
      alert("Berhasil Dihapus!");
