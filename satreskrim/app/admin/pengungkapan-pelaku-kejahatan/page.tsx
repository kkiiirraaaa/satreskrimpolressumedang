...
type PengungkapanPelakuKejahatanType = {
  id: number;
  judul: string;
  deskripsi: string;
...
export default function PengungkapanPelakuKejahatan() {
  const [PengungkapanPelakuKejahatan, setPengungkapanPelakuKejahatan] = useState<PengungkapanPelakuKejahatanType[]>([]);
  const [judul, setJudul] = useState("");
  const [deskripsi, setDeskripsi] = useState("");
  const [created_at, setCreated_At] = useState("");
  const [foto, setFoto] = useState<File | null>(null);
  const [showModal, setShowModal] = useState(false);
  const [selected, setSelected] = useState<PengungkapanPelakuKejahatanType | null>(null);
  ...
  const fetchData = async () => {
    ...
    else setPengungkapanPelakuKejahatan(data || []);
  };
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (insertError) {
        ...
        alert("Pengungkapan Pelaku Kejahatan Baru Berhasil Ditambahkan!");
        setShowModal(false);
        setJudul("");
        setDeskripsi("");
        setFoto(null);
  ...
  const handleDelete = async (id: number) => {
    ...
    if (error) {
      ...
      setPengungkapanPelakuKejahatan((prev) => prev.filter((item) => item.id !== id));
    }
    if (!error) {
      alert("Berhasil Dihapus!");
  ...
  return (
    <div className="bg-gray-100 pl-80 pt-30 p-6 min-h-screen">
      ...
      >
        Tambah Pengungkapan Pelaku Kejahatan
      </button>
            ...
            <div className="relative bg-white rounded-lg shadow-sm">
              <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-gray-200">
                <h3 className="text-lg font-semibold text-gray-900">
                  Tambah Pengungkapan Pelaku Kejahatan Baru
                </h3>
                <button
        ...
        >
          <div className="relative p-4 w-full max-w-md max-h-full">
            <div className="relative bg-white rounded-lg shadow-sm">
              <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-gray-200">
                <h3 className="text-lg font-semibold text-gray-900">
                  Edit Pengungkapan Pelaku Kejahatan
                </h3>
                <button
